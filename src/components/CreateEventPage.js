...
const CreateEventPage = () => {
    ...
    async function fetchPlanners() {
      ...
      try {
        const { data, error } = await supabase
          ...
          .select('*')
          .eq('user_id', adminId)
          .not('admin_email', 'is', null)  // Only get manual planners
  ...
  const handleCreateEvent = async () => {
    ...
    try {
      ...
      // Send notifications to all suppliers about the new event opportunity
      try {
        // Fetch all supplier emails from profiles table
        ...
        if (!suppliersError && suppliers && suppliers.length > 0) {
          ...
          if (notificationError) {
            console.error('Error creating supplier notifications:', notificationError);
            // Don't fail the event creation if notifications fail
          } else {
            console.log(`Successfully created ${notifications.length} notifications for new event: ${formData.name}`);
          }
        }
      } catch (notificationErr) {
        console.error('Error sending notifications to suppliers:', notificationErr);
        // Don't fail the event creation if notifications fail
      }
