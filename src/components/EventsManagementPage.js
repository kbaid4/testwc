  ...
  useEffect(() => {
    ...
    const fetchEventSuppliers = async (evId) => {
      try {
        ...
        // For each invite, try to get the supplier information and event_suppliers status
        ...
        for (const invite of invites) {
          ...
          // Check if supplier has accepted the invite in event_suppliers
          ...
          if (!profilesError && profiles && profiles.length > 0) {
            ...
            supplierDetails.push({
              ...
              name: profiles[0].company_name || profiles[0].full_name || invite.supplier_email,
              serviceType: profiles[0].serviceType || '',
              // Show 'Accepted' only if they've accepted in event_suppliers
              status: eventSupplier?.status === 'accepted' ? 'Accepted' : 'Pending'
            });
          } else {
            // Supplier hasn't signed in yet
            supplierDetails.push({
              email: invite.supplier_email,
    ...
    if (adminId && eventId) {
      const fetchNotifications = async () => {
        const { data, error } = await supabase
          ...
          .eq('event_id', eventId)
          .eq('type', 'application') // Only fetch supplier application notifications
                  ...
                  {eventSuppliers.length > 0 ? (
                    eventSuppliers.map((supplier, index) => (
                      <div key={index} style={{
                        ...
                        {supplier.status && (
                          <span style={{ color: '#666', fontSize: '12px', marginLeft: '5px' }}>({supplier.status})</span>
                        )}
                      </div>
                    ))
                        ...
                        <button
                          ...
                          onClick={async () => {
                            try {
                              ...
                              const eventName = currentEvent?.name || 'an event';
                              ...
                              const notificationPayload = {
                                ...
                                status: 'unread',
                                content: `Your application for "${eventName}" has been accepted! You can now view event details and tasks.`,
