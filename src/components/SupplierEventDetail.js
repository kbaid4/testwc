...
const SupplierEventDetail = () => {
  ...
  // Check if the supplier needs to accept the invite
  const checkInviteStatus = async () => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return;

      // Check event_suppliers status
      ...
      // 1. No event_supplier entry exists, or
      // 2. Status is 'pending' or null
      setShowAcceptButton(!eventSupplier || !eventSupplier.status || eventSupplier.status === 'pending');
    } catch (error) {
  ...
  const handleAcceptInvite = async () => {
    try {
      ...
      // Get supplier profile information for the notification
      let supplierName = user.email;
      try {
        ...
        if (supplierProfile) {
          supplierName = supplierProfile.company_name || supplierProfile.full_name || user.email;
        }
      } catch (profileError) {
      ...
      // Update event_suppliers status to accepted
      ...
      if (adminId) {
        try {
          const notificationData = {
            ...
            event_id: eventId,
            supplier_email: user.email, // Add the required supplier_email field
            type: 'invite_accepted',
            content: `${supplierName} has accepted the invite for "${eventName}"`,
            status: 'unread',
          ...
          if (notificationError) {
            ...
            console.log('✅ Successfully created notification for admin:', {
              type: notificationData.type,
              content: notificationData.content,
              admin_user_id: notificationData.admin_user_id,
              event_id: notificationData.event_id
            });
          }
        } catch (notificationErr) {
          console.error('Error in notification creation:', notificationErr);
          // Don't fail the accept process if notification fails
        }
      }

      // Recheck invite status to update button visibility
      await checkInviteStatus();

      // Show success message
      alert('You have successfully accepted the invite!');
    } catch (error) {
        ...
        {loading ? (
          ...
          <div className="event-detail-card" style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '15px' }}>
              ...
              {(eventDetails.status && eventDetails.status.toLowerCase() !== 'invited') && (
                <span style={{
            ...
            <div className="event-actions" style={{ marginTop: '30px', display: 'flex', gap: '15px', flexWrap: 'wrap' }}>
              <button
