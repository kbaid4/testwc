  ...
  useEffect(() => {
    async function fetchSuppliers() {
      ...
      const { data: allSuppliers, error: profilesError } = await supabase
        .from('profiles')
        .select('id, full_name, email, company_name, created_at, user_type, service_type')
      ...
      const signedUpSuppliersArrCamel = signedUpSuppliersArr.map(s=>({...s, serviceType: s.service_type}));
      setSignedUpSuppliers(signedUpSuppliersArrCamel);
      localStorage.setItem('signedUpSuppliers', JSON.stringify(signedUpSuppliersArrCamel));
      ...
      if (taskData.searchedSupplier) {
        ...
        if (supplierEmail) {
          ...
          try {
            ...
            // 3. Insert or update event_suppliers entry with pending status
            const { data: existingEventSupplier } = await supabase
              .from('event_suppliers')
              .select('id')
              .eq('event_id', eventId)
              .eq('supplier_email', supplierEmail);

            if (!existingEventSupplier || existingEventSupplier.length === 0) {
              await supabase.from('event_suppliers').insert([
                {
                  event_id: eventId,
                  supplier_email: supplierEmail,
                  status: 'pending',
                        ...
                        {signedUpSuppliers.map(supplier => {
                          ...
                          return (
                            <option key={supplier.id} value={supplier.id}>
                              {serviceType ? `${displayName} (${serviceType})` : displayName}
                            </option>
                          );
                        })}
                      </optgroup>
