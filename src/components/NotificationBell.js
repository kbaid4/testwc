...
const NotificationBell = ({ userType, userId, supplierEmail }) => {
  ...
  const shouldHideNotification = (notification, userType) => {
    ...
    // For message notifications, show only relevant ones based on user type
    if (notification.type === 'new_message') {
      const content = notification.content || '';
      if (userType === 'admin') {
        // Admin should only see messages from suppliers
        return !content.includes('from supplier:');
      } else if (userType === 'supplier') {
        // Supplier should only see messages from admin
        return !content.includes('from admin:');
      }
    }
  ...
  const fetchNotifications = async () => {
    try {
      ...
      if (data) {
        ...
        // Count unread notifications after filtering
        const unread = filteredData.filter(notification => notification.status === 'unread').length;
