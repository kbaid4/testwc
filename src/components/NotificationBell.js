...
const NotificationBell = ({ userType, userId, supplierEmail }) => {
  ...
  const shouldHideNotification = (notification, userType) => {
    ...
    // Note: invite_accepted notifications are NOT hidden from admins - they should see when suppliers accept invites
    
    // Hide invite_accepted notifications from suppliers (these are for admins only)
    if (userType === 'supplier' && notification.type === 'invite_accepted') {
      return true;
    }
    ...
    try {
      ...
      if (userType === 'admin' && userId) {
        ...
        query = supabase
          ...
          // Note: invite_accepted notifications are NOT excluded - they should be shown to admins
  ...
  const formatNotificationMessage = (notification) => {
    ...
    // Invite accepted notifications for admins only
    if (notification.type === 'invite_accepted') {
      if (userType === 'admin') {
        // Use the content directly as it already contains the formatted message
        return notification.content || 'A supplier has accepted your invite';
      } else {
        // Return null for suppliers - this notification should not be shown to them
        return null;
      }
    }
